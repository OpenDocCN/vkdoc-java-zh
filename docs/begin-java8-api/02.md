...
表 2-15 列出了`JList`类的常用方法。注意，`JList`没有一个直接的方法来给出列表的大小(T2 中选择的数量)。由于每个 Swing 组件都使用一个模型，所以`JList`也是如此。它的模型是`JListModel`接口的一个实例。要知道一个`JList`的选择列表的大小，您需要调用它的模型的`getSize()`方法，就像这样:
