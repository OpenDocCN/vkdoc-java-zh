...
JNI 定义了 Java 中使用的数据类型和本地函数之间的映射。表 [8-3 列出了 Java 和本地 C/C++语言之间的原始数据类型映射。请注意，在 Java 中，您只需在原始数据类型的名称前添加一个`j`,就可以在 C/C++中获得等效的数据类型名称。JNI 还定义了一个名为`jsize`的数据类型，用来存储长度，比如数组或者字符串的长度。
...
JNI 定义了 Java 引用类型的引用类型等价物。不可能在 JNI 中为所有可以在 Java 中创建的引用类型定义单独的类型。所有 Java 引用类型都可以映射到名为`jobject`的 JNI 引用类型。你有一些专门的 JNI 引用类型，代表 Java 中常用的引用类型，比如 JNI 的`jstring`代表 Java 中的`java.lang.String`。表 8-4 列出了 Java 和 JNI 之间的引用类型映射。
