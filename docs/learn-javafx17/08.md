...
清单 8-8 有完整的程序来演示样式类选择器`hbox`和`button`的使用。产生的屏幕如图 8-7 所示。
...
运行清单 8-10 中的程序，看看这些变化的效果。请注意，当您在场景中的任何地方移动鼠标时，除了在名称文本字段上，您会得到一个`HAND`光标。这是因为`TextField`类覆盖了`-fx-cursor` CSS 属性，将其设置为`TEXT`光标。
