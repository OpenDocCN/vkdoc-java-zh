...
最后，注意清单 13-4 中的`getStepCount`方法。它不在前面提到的需求或接口中。在玩了`Step`(清单 13-1 )并思考了清单 13-4 中的`run`方法的含义后，我意识到类似于`getStepCount`的东西对于理解“T4 然后是`stop`场景中的步进电机定位非常有用。我请求 diozero 的开发者在`DigitalOutputDevice`中插入必要的逻辑。
