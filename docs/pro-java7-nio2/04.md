...
![Image](img/square.jpg) **注意**`move()`方法也可以用来移动空目录。试图移动一个非空的目录通常是一个递归复制操作，正如你在第五章中看到的。然而，如果不需要移动目录中的条目，也可以移动非空的目录。在某些情况下，目录包含创建目录时创建的特殊文件(如链接)的条目，如果目录只包含这些条目，则认为它是空的。
