...
Java 中有几种类型的代码片段，例如，变量声明、带初始化的变量声明、方法声明、类型声明等。`Snippet`类是一个抽象类，有一个子类来表示每个特定类型的代码片段。图 23-6 显示了`Snippet`类及其后代的类图。
...
让我们看看 JShell API 的实际应用。清单 23-5 包含了一个名为`JShellApiTest`的类的完整代码，它是`jdojo.jshell`模块的成员。
