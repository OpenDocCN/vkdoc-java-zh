...
可以使用完全限定的类名来代替 Java import 关键字，也就是说，在 Java 代码中，类名的前面加上包名。惯例规定使用 import 语句；然而，如果你想打破标准 Java 编程惯例，图 3-2 中的第 20 行可以写成`javafx.scene.control.Button btn = new javafx.scene.control.Button();`。
...
由于 Java 中的类继承概念，公共类中的所有公共方法和公共变量都将被该类的子类继承(一旦被子类化，就成为超类)。图 3-2 在`InvinciBagel`类关键字前面提供了一个公共访问控制修饰符关键字的例子。
