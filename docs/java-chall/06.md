...
如前所述，以毫秒为单位表示时间信息，有助于计算机处理，但与人类的思维方式和他们在时间系统中的取向关系不大。人类更喜欢用时间段或循环日期来思考，例如，12/24 代表平安夜，12/31 代表新年前夜，等等。(即没有时间和年份的日期)。有时你需要*不完整的时间信息*，比如不涉及日期的时间，比如下班后 6 点，或者作为组合，比如周二和周四晚上 7 点的空手道训练。 <sup>1</sup> 用 Java 8 之前就有的 API 表达类似的东西是相当困难的。现在让我们来看看自 JDK 8 以来的可能性。
...
类`java.time.Duration`允许以纳秒为单位指定持续时间。类别`Duration`的实例可以通过调用各种方法(例如，从不同时间单位 <sup>2</sup> 的值)来构建，如下所示:
...
当查看这个输出时，您可能会对`Duration`的字符串表示感到恼火。乍一看，这似乎有点不寻常。然而，它遵循 ISO 8601 标准。输出总是以缩写`PT`开始。 <sup>3</sup> 之后是小时(`H`)、分钟(`M`)、秒(`S`)的分段。如果需要，毫秒甚至纳秒都显示为十进制数。
